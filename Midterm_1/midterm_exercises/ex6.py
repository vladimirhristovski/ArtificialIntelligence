...
class Squares(Problem):
    ...
    def move(self, test, direction, state, pos):
        square = list(test)
        state_list = list(state)

        if pos == 0:
            index = state_list.index(test)
        else:
            for i in range(len(state_list)):
                if state_list[i] == test:
                    index = i

        square[0] += direction[0]
        square[1] += direction[1]

        state_list[index] = tuple(square)

        if not self.check_valid(tuple(state_list)):
            return None
    ...
    def successor(self, state):
        ...
        new_stete = list(state)

        actions = (" gore", " dolu", " levo", " desno")
        directions = ((0, +1), (0, -1), (-1, 0), (+1, 0))

        for i in range(len(new_stete)):
            for action, direction in zip(actions, directions):
                res = self.move(new_stete[i], direction, state, i)
                if res is not None:
                    # print(res, "Pomesti kvadratche " + str(i + 1) + action)
                    succ["Pomesti kvadratche " + str(i + 1) + action] = res
...
if __name__ == '__main__':
    ...
    node = breadth_first_graph_search(squares)

...
class GhostOnSkates(Problem):
    ...
    def move(self, state, direction):
        ...
        if 0 > ghost[0] <= self.n or 0 > ghost[1] <= self.n or tuple(ghost) in self.walls:
            return None

        new_state = tuple(ghost)
    ...
    def successor(self, state):
        ...
        for action, direction in zip(actions, directions):
            ...
            if res is not None:
                successors[action] = res

        return successors

    def h(self, node):
        g_x, g_y = node.state
        return (abs(g_x - self.goal[0]) + abs(g_y - self.goal[1])) / 3

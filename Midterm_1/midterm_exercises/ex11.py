...
class Snake(Problem):
    ...
    def __init__(self, green_apples):
    ...
    def h(self, node):
        ...
        snake = state[0]
        apples = state[2]
        # return self.lowest_m(snake[-1], apples)
        return self.my_way(snake[-1], apples)

    def my_way(self, start, apples):
        if len(apples) == 0:
            return 0
        total = self.manhattan(start, apples[0])
        # total =0
        explore = list(apples)
        for e in explore:
            if self.manhattan(start, e) != 0 and self.manhattan(start, e) < total:
                total = self.manhattan(start, e)
            # total += self.manhattan(start, e)
        return total

    def lowest_m(self, start, apples):
        new_apples = list(apples)
        if len(apples) == 0:
            return 0
        value = self.manhattan(start, apples[0])
        a = apples[0]
        for apple in apples:
            if self.manhattan(start, apple) < value:
                value = self.manhattan(start, apple)
                a = apple
        new_apples.remove(a)
        return value + self.lowest_m(a, tuple(new_apples))
...
if __name__ == '__main__':
    ...
    problem = Snake(tuple(green_apples))
